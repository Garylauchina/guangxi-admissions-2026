...
def parse(path):
    p=NestedTables();p.feed(Path(path).read_text());return p.tables

class NestedTables(Tables):
    """Keep a separate table context for genuine tables nested inside CMS layouts."""
    def __init__(self):
        super().__init__();self.stack=[]
    def handle_starttag(self,t,a):
        if t=='table':
            if self.depth:self.stack.append((self.depth,self.rows,self.row,self.cell,self.context))
            self.depth=0;self.rows=[];self.row=None;self.cell=None
        super().handle_starttag(t,a)
    def handle_endtag(self,t):
        super().handle_endtag(t)
        if t=='table' and self.stack:self.depth,self.rows,self.row,self.cell,self.context=self.stack.pop()
