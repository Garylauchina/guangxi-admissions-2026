...
def get(item):
    ...
    meta={'id':key,'url':url,'checkedAt':datetime.now(timezone.utc).isoformat(),'requestMethod':item.get('method','GET'),'parameters':item.get('parameters',{})}
    try:
        with urlopen(Request(url,data=urlencode(item['parameters']).encode() if item.get('method')=='POST' else None,headers={'User-Agent':'Mozilla/5.0'}),timeout=35) as r:
