...
def fetch(item):
    ...
    with lock:
        ...
        meta['finishedAt']=datetime.datetime.now(datetime.timezone.utc).isoformat()
        if 'sha256' not in meta: meta['errorSha256']=hashlib.sha256(meta.get('error','').encode()).hexdigest()
