...
from collector import host_request_slot
...
def strip_session(obj):
    sensitive = {'jessionid','jsessionid','sessionid','session','token','csrftoken','csrf-token','cookie','set-cookie','seal'}
...
def error(job,exc):
    ...
    if hasattr(exc,'read'):
        ...
        original_body=body
        if body:
            try: body=(json.dumps(strip_session(json.loads(body)),ensure_ascii=False,indent=2)+'\n').encode()
            ...
            meta.update(rawFile=name,sha256=digest,responseSha256=hashlib.sha256(original_body).hexdigest(),byteCount=len(body))
    (RAW/f"{job['id']}.meta.json").write_text(json.dumps(meta,ensure_ascii=False,indent=2)+'\n')
    return meta
...
def fetch(job):
    try:
        ...
        headers.update(job.get('publicHeaders', {}))
        data=None
        ...
        with host_request_slot(job['url']):
...
class PublicAPI:
    def __init__(self,host,entry):
        ...
        with host_request_slot(entry):
            with self.opener.open(urllib.request.Request(entry,headers={'User-Agent':'Mozilla/5.0'}),timeout=35) as response:
                response.read()
    def req(self,path,data,token=False):
        with host_request_slot(self.host):
