...
if __name__=='__main__':
    ...
    for name in ['jnu','jxpu']:
        a=parse(R/'evidence'/f'{name}.html')
        (R/'evidence'/f'{name}-tables.json').write_text(json.dumps(a,ensure_ascii=False,indent=2)+'\n')
        for i,t in enumerate(a):print(name,i,len(t['rows']),t['context'][-80:],t['rows'][:3])
