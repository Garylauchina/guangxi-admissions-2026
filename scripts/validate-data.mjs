...
const planSources=await read('plan-source-catalog');
assert.equal(new Set(planSources.map(r=>r.schoolCode)).size,planSources.length,'Duplicate plan-source school');
for(const row of planSources){assert.equal(row.year,2026);assert.match(String(row.schoolCode),/^\d{5}$/);assert.ok(row.school&&row.note&&row.checkedAt&&row.sourceIds.length);assert.ok(['collected','source-found','entry-only','unavailable'].includes(row.status));validateNestedSources(row);if(row.entryUrl)assert.match(row.entryUrl,/^https?:\/\//);if(row.status==='collected')assert.ok(plans.some(p=>String(p.schoolCode)===String(row.schoolCode)));}
for(const row of plans){assert.ok(planSources.some(s=>String(s.schoolCode)===String(row.schoolCode)),`Missing plan-source catalog: ${row.school}`);assert.ok(!row.groupEvidenceRound||['首轮','初始计划'].includes(row.groupEvidenceRound),'Other-round group evidence cannot establish initial grouping');assert.ok(!/征集/.test(`${row.round||''} ${row.evidenceRound||''}`));assert.notEqual(row.firstRoundMatchAllowed,false);assert.notEqual(row.initialPlanEligible,false);assert.notEqual(row.plannedCountScope,'remaining-supplementary-plan');if(row.planStage)assert.equal(row.planStage,'initial');if(row.initialPlanGroupVerified===false)assert.equal(row.group,null);}
const supplementaryPlans=await read('supplementary-plans');
const supplementaryIds=new Set();
for(const row of supplementaryPlans){assert.equal(row.year,2026);assert.ok(!ids.has(row.id)&&!supplementaryIds.has(row.id),'Supplementary plans must be distinct');supplementaryIds.add(row.id);assert.ok(['物理','历史'].includes(row.track)&&row.major&&exactGroupKey(row));assert.ok(/征集/.test(row.round));assert.equal(row.plannedCountScope,'remaining-supplementary-plan');assert.equal(row.firstRoundMatchAllowed,false);assert.ok(Number.isInteger(row.remainingCount)&&row.remainingCount>=0);assert.equal(row.plannedCount,row.remainingCount);validateNestedSources(row);}
