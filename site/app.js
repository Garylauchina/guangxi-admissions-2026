...
import { PLAN_GAPS, planGaps, isComparableMajorScore, comparableMajorCount, groupAdmissionIndex, groupAdmissionFor } from './coverage.js?v=525939462aab';

const $ = id => document.getElementById(id);
const fmt = n => Number(n).toLocaleString('zh-CN');
const YEAR = 2026;
const DATA_VERSION = '006e0b15230b457d91c9728e2d8f3b8ae23224a37621264dc4c3ad24a81c61a2';
...
Object.assign(scoreAuditLabels, {'prior-year-only':'仅取得往年专业分', 'year-unavailable':'查询目录未提供本年专业分', 'reviewed-gap':'已核查，仍缺可比较专业分', 'filing-only':'仅有专业投档分，实际录取分仍缺', 'other-route-only':'仅取得其他招生通道资料', 'outcome-without-scores':'已公布录取进展，未提供专业分', 'schedule-only':'仅取得录取日程信息'});
...
let data, sourceMap, allRows, groupIndex, planIndex, supplementaryPlanIndex, schoolAuditIndex, majorReviewStats, admissionIndex, filtered = [], page = 1, view = 'cutoffs', reference = {score:null};
let saved = new Set();
let gapPage = 1;
...
const reviewedForMajorScores = row => hasMajorScoreReview(row) || (row.majorCutoffs || 0) > 0;
function sourceLinks(row) { return sourceIds(row).map(id => sourceLink(id)).join(' · '); }
function embeddedRecords(records, kind, title) {
  if (!records.length) return '';
  return `<details class="embedded-records"><summary>${h(title)}（${records.length} 条）</summary>${records.map(r => `<div class="detail-plan" data-embedded-record="${h(r.id)}"><strong>${h(r.major)}</strong><p>${h(r.track)}类 · ${h(r.batch)} · ${h(r.round || '轮次待核')}${r.group ? ' · '+h(r.group)+' 组' : ' · 组码待核'} · ${h(specialLabel(r))}</p><p>${kind === 'plans' ? `${r.currentGaokaoSeatsKnown===false ? '公布人数（含直升，非已确认可填人数）' : '计划'} ${h(r.plannedCount ?? '待核实')} 人 · ${h(r.requirementText || '选科要求待核实')} · 学费 ${h(r.tuition ?? '待核实')}` : `专业录取最低分：${isComparableMajorScore(r) ? h(r.score)+' 分' : '原文待核（不参与比较）'}`}</p><div class="record-actions"><button type="button" data-detail="${h(r.id)}">专业详情</button><button type="button" data-save="${h(r.id)}" aria-pressed="${saved.has(r.id)}">${saved.has(r.id) ? '已加备选' : '＋ 加入备选'}</button>${sourceLinks(r)}</div></div>`).join('')}</details>`;
}
...
function schoolMajorScores(row) {
  ...
  const comparable = records.filter(isComparableMajorScore).length, pending = records.length - comparable;
  const reviews = schoolAuditNotes(row).filter(isMajorScoreAudit);
  return `<section class="plan-source-panel"><h3>该校专业录取分数</h3><p>${records.length ? `本站该校${h(row.track)}类有 ${comparable} 条可比较专业录取分、${pending} 条原文待核。${comparable ? '含不同批次、招生类别和录取轮次；不代表本组已全部匹配。' : '已核查但尚无可比较专业分；待核记录不参与分数比较。'}` : `尚未收录该校${h(row.track)}类的专业实际录取分，不能用本组投档线代替。`}</p>${embeddedRecords(records.filter(r => exactGroupKey(row) && exactGroupKey(r) === exactGroupKey(row)), 'scores', '已核实属于本组的专业录取分（保留各轮次）')}${embeddedRecords(records.filter(r => !exactGroupKey(row) || exactGroupKey(r) !== exactGroupKey(row)), 'scores', '本校同科类其他专业分（未确认为本组）')}${records.length ? `<p><button type="button" data-school-query="${h(row.schoolCode)}" data-destination="majorCutoffs" data-track="${h(row.track)}">${comparable ? '查看该校专业分' : '查看待核专业分'}</button></p>` : ''}${reviews.length ? `<details><summary>查看专业分核查结果与来源</summary>${reviews.map(n=>`<p>${h(n.note)}</p><p class="tiny muted">核查：${h(reviewDate(n.checkedAt))} · ${h(auditStatusLabel(n))}</p><p>${(n.sourceIds || []).map(id=>sourceLink(id,sourceMap.get(id)?.title || '复查来源 ↗')).join('<br>')}</p>`).join('')}</details>` : '<p class="tiny muted">尚无该校独立分数来源复查记录，不能据此判断学校是否已公布。</p>'}</section>`;
}
function schoolMajorFiling(row) {
  const records = data.majorFiling.filter(r => String(r.schoolCode) === String(row.schoolCode) && r.track === row.track);
  if (!records.length) return '';
  return `<section class="plan-source-panel major-filing-panel"><details><summary>专业投档分（不是实际专业录取分）：该校${h(row.track)}类 ${records.length} 条</summary><p class="tiny">以下是高校原文公布的专业投档记录，含不同批次、招生类别及轮次；不代表当前专业组的完整名单，也不进入专业实际录取分筛选或分差比较。</p>${records.map(r => `<div class="detail-plan" data-major-filing="${h(r.id)}"><strong>${h(r.major)}</strong><p>${h(r.batch || '批次待核')} · ${h(r.round || '轮次待核')}${r.admissionType ? ' · '+h(r.admissionType) : ''}</p><p>${h(r.scoreType || '专业投档最低分')}：${r.scoreComparable === false ? `原文待核（来源记载 ${h(r.score ?? '未公布')} 分，不参与比较）` : Number.isFinite(r.score) ? `${h(r.score)} 分` : '未公布'}</p>${r.scoreEvidenceGaps?.length ? `<p class="gap-warning">${h(r.scoreEvidenceGaps.join('；'))}</p>` : ''}${r.note ? `<p class="tiny">${h(r.note)}</p>` : ''}<p>${sourceLinks(r)}</p>${r.sourceLocator ? `<p class="tiny muted">原文定位：${h(r.sourceLocator)}</p>` : ''}</div>`).join('')}</details></section>`;
}
function initialize() {
  ...
  const majorSchools = new Set(data.majorCutoffs.filter(isComparableMajorScore).map(row => String(row.schoolCode || row.school)));
  const allMajorSchools = new Set(data.majorCutoffs.map(row => String(row.schoolCode || row.school)));
  const hasMajorRecords = row => majorSchools.has(String(row.schoolCode || row.school));
  const reviewed = row => hasMajorScoreReview(row) || allMajorSchools.has(String(row.schoolCode || row.school));
  majorReviewStats = {
    total: reviewSchools.length,
    collected: reviewSchools.filter(hasMajorRecords).length,
    reviewed: reviewSchools.filter(reviewed).length,
    reviewedWithoutRecords: reviewSchools.filter(row => !hasMajorRecords(row) && reviewed(row)).length,
    notReviewed: reviewSchools.filter(row => !hasMajorRecords(row) && !reviewed(row)).length,
    comparableRecords: data.majorCutoffs.filter(isComparableMajorScore).length,
    pendingRecords: data.majorCutoffs.filter(row => !isComparableMajorScore(row)).length,
    pendingSchools: new Set(data.majorCutoffs.filter(row => !isComparableMajorScore(row)).map(row => String(row.schoolCode || row.school))).size,
    pendingOnlySchools: [...allMajorSchools].filter(code => !majorSchools.has(code)).length,
  };
  groupIndex = firstRoundIndex(data.cutoffs);
  admissionIndex = groupAdmissionIndex(data.groupAdmissions);
...
function render() {
  ...
  $('result-notice').innerHTML = isPlan
    ...
    : isMajor
    ? '<strong>这里展示高校公开的具体专业录取最低分，现为部分覆盖。</strong> 年份或其他原文冲突另标“原文待核”，不参与分数比较。可按录取轮次缩小范围；每条保留首轮、征集或汇总口径，汇总不能当作首轮。未公布组码不猜测。加分处理以高校口径为准，不能一律视作裸分。'
    : `<strong>分数筛选及排序按投档线。</strong> 最低录取分仅采用高校直接公布的本组数据，录取轮次单独注明；缺口显示待补齐。专业关键词搜索已核实组码的已收录计划。${$('round').value === '首轮' ? '分数为空表示原表无出档考生。' : '正在查看征集投档，与首次投档分开比较。'}`;
  if (!filtered.length) $('results-list').innerHTML = `<div class="empty-state"><strong>${reference.error ? '请先修正输入' : '当前条件下没有结果'}</strong><p>${reference.error ? h(reference.error) : '试试清空关键词、放宽分差或切换“全部已收录批次”。'}</p><p class="tiny">未收录或未匹配专业计划，不代表该校没有招生。</p>${view==='cutoffs'&&$('query').value?'<button type="button" class="outline-button" data-search-plans>在已收录专业资料中继续搜索</button>':''}</div>`;
...
function card(r) {
  ...
  const admission = !isPlan && !isMajor ? groupAdmissionFor(r, admissionIndex) : null;
  const admittedScore = admission && admission.scoreComparable !== false && Number.isFinite(admission.score) ? admission.score : null;
  let major = '';
  if (isPlan) {
  ...
  return `<article class="result-card" data-record="${h(r.id)}"><div><div class="school-line"><h3 class="school-name">${h(r.school)}</h3>${r.group ? `<span class="group-tag">${h(r.group)} 组</span>` : '<span class="group-tag">组码待核</span>'}</div><p class="record-meta">${h(r.schoolCode || '代码待核')} · ${h(r.track)}类 · ${h(r.batch)}${r.round ? ' · '+h(r.round) : ''} · ${h(specialLabel(r))}</p>${major}${r.note ? `<p class="record-note">${h(r.note)}</p>` : ''}<div class="record-actions"><button type="button" data-detail="${h(r.id)}">查看详情</button><button type="button" data-save="${h(r.id)}" class="${saved.has(r.id)?'saved':''}" aria-pressed="${saved.has(r.id)}">${saved.has(r.id)?'已加备选':'＋ 加入备选'}</button>${sourceLinks(r)}</div></div>${!isPlan && !isMajor ? '<div class="score-pair">' : ''}<div class="score-box filing-score"><strong class="score ${score === null ? 'empty' : ''}">${score === null ? (r.scoreComparable===false ? '原文待核' : isPlan ? '待匹配' : '未公布') : h(score)}</strong><div class="score-label">${isMajor ? '专业录取最低分' : isPlan ? '2026 首轮组线' : '2026年投档线'}${isPlan ? '<br>不是专业录取分' : ''}</div>${diff !== null ? `<span class="diff ${diff<0?'negative':''}">${diff >= 0 ? '高于线 ' : '低于线 '}${Math.abs(diff)} 分</span>` : ''}</div>${!isPlan && !isMajor ? `<div class="score-box admission-score"><strong class="score ${admittedScore === null ? 'empty' : ''}">${admittedScore === null ? (admission ? '待核实' : '待补齐') : h(admittedScore)}</strong><div class="score-label">2026年最低录取分</div><p class="tiny muted">${admission ? admission.roundScope === 'annual' ? '全年录取汇总 · 轮次未分' : h(admission.round) : '尚缺本组完整录取依据'}</p></div></div>` : ''}</article>`;
}
...
function openDetail(id) {
  ...
  const rows = [[r.conflictFields?.includes('year') ? '检索年份（原文年份冲突待核）' : '数据年份',r.year],['科类 / 批次',`${r.track}类 / ${r.batch}`],['专业组代码',r.group || '待核实'],['投档或录取轮次',r.round || '见高校来源说明'],[r.scoreComparable===false ? '来源记载最低分（冲突待核，不参与比较）' : '最低分',r.score ?? group?.score ?? '未公布 / 未匹配'],['分数口径',data.plans.some(p=>p.id===r.id) ? '同组首次投档线，不是专业录取线' : r.scoreType || '院校专业组投档最低分']];
  if (isCutoff) {
    rows[4][0] = '2026年投档线';
    const admission = groupAdmissionFor(r, admissionIndex);
    rows.push(['2026年最低录取分', admission && admission.scoreComparable !== false ? admission.score : '待补齐'], ['录取分统计范围', admission ? admission.roundScope === 'annual' ? '本专业组全年录取汇总，轮次未分' : admission.round : '尚缺本组完整录取依据']);
  }
  ...
  $('detail-content').innerHTML = `<h2>${h(r.school)}${r.group?' · '+h(r.group)+' 组':''}</h2><div class="detail-grid">${rows.map(([k,v])=>`<div><span>${h(k)}</span><strong>${h(v)}</strong></div>`).join('')}</div>${r.note ? `<div class="notice">${h(r.note)}</div>` : ''}${r.major && r.requiredSubjects ? `<p>再选科目：${r.subjectRule === 'unknown' ? '待核实' : r.requiredSubjects.length ? h(r.requiredSubjects.join(r.subjectRule==='any'?' / ':' + ')) : '不限'}。学费：${h(r.tuition ?? '待核实')}；学制：${h(r.duration ?? '待核实')}。</p>` : ''}${gaps.length ? `<div class="notice"><strong>本专业尚待核实或关联：</strong>${h(gaps.join('、'))}。可在“院校数据缺口”查看该校资料覆盖。</div>` : ''}${showGroupPlans ? `<h3>${initialPlans ? '已采集并匹配到本组的专业计划' : '本轮征集剩余计划'}（${plans.length} 条）</h3><p class="tiny muted">${initialPlans ? '目前是部分计划覆盖；填报前需确认全组专业及调剂范围。' : h(r.round)+'公布的剩余名额，不是初始计划人数，也不代表目前仍可填报。'}</p>${plans.length ? plans.map(p=>`<div class="detail-plan"><strong>${h(p.major)}</strong><p>${!initialPlans ? '本轮剩余' : p.currentGaokaoSeatsKnown===false ? '公布人数（含直升，非已确认可填人数）' : '计划'} ${h(p.plannedCount ?? '待核实')} 人 · 学费 ${h(p.tuition ?? '待核实')} · 学制 ${h(p.duration ?? '待核实')}</p><p>${h(p.note || '')}</p>${sourceLinks(p)}</div>`).join('') : initialPlans ? '<p>本组还没有可严格匹配的专业计划：可能尚未采集，也可能已取得该校计划但缺少本组对应关系。请查看下方来源，不用相似专业或其他轮次推定本组计划。</p>' : '<p>本组本轮征集计划尚未采集，不使用首轮或其他轮次计划代替。</p>'}` : ''}${isCutoff || isPlan ? schoolPlanSources(r) + schoolMajorScores(r) + schoolMajorFiling(r) : ''}<h3 style="margin-top:24px">来源与核验</h3>${sourceIds(r).map(id=>{const s=sourceMap.get(id);return s ? `<p>${sourceLink(id,s.title)}<br><span class="tiny muted">发布：${h(s.publishedAt || '原页未标日期')} · 采集：${h(reviewDate(s.accessedAt) || '未标')} ${r.sourceRow && id === r.sourceId ? '· 原表数据行 '+h(r.sourceRow) : ''}</span></p>${s.notes?.length ? `<p class="tiny">${h(s.notes.join(' '))}</p>` : ''}` : '';}).join('')}${fieldEvidence ? `<h3>补充字段的核验来源</h3>${fieldEvidence}` : ''}${group ? `<p>组投档线来源：${sourceLinks(group)}</p>` : ''}${isCutoff && groupAdmissionFor(r, admissionIndex) ? `<p>本组最低录取分来源：${sourceLinks(groupAdmissionFor(r, admissionIndex))}</p>` : ''}<p class="help">院校章程中的体检、单科、外语语种、校区、调剂与资格限制仍须逐项核对。2026 年数据不能保证未来录取。</p>`;
  if (!$('detail').open) $('detail').showModal();
}
function exportSaved() {
  if (!saved.size) return toast('先把感兴趣的院校或专业加入备选。');
  const header=['年份','院校','院校代码','科类','批次','轮次','专业组','专业','公布计划人数（口径见备注）','录取人数','分数','分数口径','2026年投档线','2026年最低录取分','组录取分统计范围','备注','官方来源'];
  const rows=[...saved].map(id=>allRows.get(id)).filter(Boolean).map(r=>{
    const admission=!r.major ? groupAdmissionFor(r,admissionIndex) : null;
    return [r.year,r.school,r.schoolCode,r.track,r.batch,r.round,r.group,r.major,r.plannedCount,r.admittedCount,comparableScore(r),r.major && !r.matchedGroupId ? r.scoreType || '无已匹配分数' : '专业组投档线，不是专业录取分',!r.major ? r.score : null,admission?.scoreComparable!==false ? admission?.score : null,admission ? admission.roundScope==='annual' ? '全年录取汇总，轮次未分' : admission.round : '',r.note,[...new Set([...sourceIds(r),...(admission ? sourceIds(admission) : []), ...(r.matchedGroupId ? sourceIds(allRows.get(r.matchedGroupId)) : [])])].map(id=>sourceMap.get(id)?.url || '').join(' | ')];
  });
...
function renderCoverage() {
  const first=data.cutoffs.filter(r=>r.round==='首轮'), schools=new Set(first.map(r=>r.schoolCode)).size, planSchools=new Set(data.plans.map(r=>r.school)).size, majorSchools=majorReviewStats.collected;
  $('coverage-strip').innerHTML=`<span><strong>${fmt(majorReviewStats.total)}</strong>个目标院校代码</span><span><strong>${fmt(majorReviewStats.collected)}</strong>个已有可比较专业分</span><span><strong>${fmt(majorReviewStats.reviewedWithoutRecords)}</strong>个已核查仍缺可比较分</span><span><strong>${fmt(majorReviewStats.notReviewed)}</strong>个待核查专业分</span>`;
  $('major-review-note').textContent=`专业录取分已核查 ${fmt(majorReviewStats.reviewed)} 个院校代码。已有 ${fmt(majorReviewStats.comparableRecords)} 条可比较专业录取分，另有 ${fmt(majorReviewStats.pendingRecords)} 条原文待核、不参与比较（${fmt(majorReviewStats.pendingOnlySchools)} 校仅有待核记录）。仍为部分覆盖；已核查不代表已取得全部专业分。`;
  $('coverage-detail').innerHTML=`<div class="notice"><strong>覆盖边界：</strong>普通批投档表按已列官方原表完整采集；分专业计划、专业实际录取线及特殊通道广西证据仍有缺口，本站不是全专业完整数据库。</div><table class="coverage-table"><thead><tr><th>资料</th><th>已收录</th><th>范围</th></tr></thead><tbody><tr><td>普通批首次投档</td><td>${fmt(first.length)} 条 / ${fmt(schools)} 个院校代码</td><td>本科、高职高专；物理、历史</td></tr><tr><td>普通批征集投档</td><td>${fmt(data.cutoffs.length-first.length)} 条</td><td>按轮次分开，空分保持空值</td></tr><tr><td>专业组最低录取分</td><td>${fmt(data.groupAdmissions.length)} 条 / ${new Set(data.groupAdmissions.map(r=>r.schoolCode)).size} 校</td><td>高校直接公布的组级录取统计；全年汇总与具体轮次分别标注，缺少数据不由专业分推算</td></tr><tr><td>分专业招生计划</td><td>${fmt(data.plans.length)} 条 / ${planSchools} 校</td><td><strong>部分覆盖</strong>，不是全区完整招生计划</td></tr><tr><td>征集剩余专业计划</td><td>${fmt(data.supplementaryPlans.length)} 条</td><td>部分覆盖，仅在相同轮次专业组详情中展示</td></tr><tr><td>可比较专业录取分</td><td>${fmt(majorReviewStats.comparableRecords)} 条 / ${majorSchools} 校</td><td><strong>部分覆盖</strong>，以高校公布的口径为准</td></tr><tr><td>原文待核专业分</td><td>${fmt(majorReviewStats.pendingRecords)} 条 / ${fmt(majorReviewStats.pendingSchools)} 校</td><td>其中 ${fmt(majorReviewStats.pendingOnlySchools)} 校仅有待核记录；含年份或其他原文冲突，不参与分数比较</td></tr><tr><td>一分一档</td><td>${fmt(data.ranks.length)} 个分数档</td><td>物理、历史；全国性加分口径</td></tr><tr><td>强基与竞赛通道</td><td>${data.special.filter(p=>p.type==='strong-foundation').length} 校强基 + ${data.special.filter(p=>p.type!=='strong-foundation').length} 个其他项目</td><td>全国目录与广西可报状态分开标注</td></tr></tbody></table><p class="help">最近补采或核查：${h([...data.auditNotes.map(n=>reviewDate(n.checkedAt)),...data.sources.map(s=>reviewDate(s.accessedAt))].filter(Boolean).sort().at(-1))}（北京时间）。各条记录的来源日期见详情；本次日期不代表全库已重查。计划已收录学校：${h([...new Set(data.plans.map(r=>r.school))].sort((a,b)=>a.localeCompare(b,'zh-CN')).join('、') || '正在整理')}。</p>`;
}
function renderGaps() {
  if (!data) return;
  ...
  const rows=data.coverage.filter(r=>{
    ...
    return status==='all'||(status==='collected'&&(r.plans||comparableMajorCount(r)))||(status==='no-plans'&&!r.plans)||(status==='no-majors'&&!comparableMajorCount(r))||(status==='major-collected'&&comparableMajorCount(r))||(status==='major-reviewed-gap'&&!comparableMajorCount(r)&&reviewedForMajorScores(r))||(status==='major-not-reviewed'&&!comparableMajorCount(r)&&!reviewedForMajorScores(r))||(status==='major-pending-only'&&r.majorCutoffs&&!comparableMajorCount(r))||(status==='subjects'&&r.planGaps.subjects)||(status==='reviewed'&&notesFor(r).length);
  ...
  const noPlans=data.coverage.filter(r=>!r.plans).length, noMajors=data.coverage.filter(r=>!comparableMajorCount(r)).length;
  $('gap-overview').textContent=`全量核查范围 ${fmt(majorReviewStats.total)} 个院校代码：${fmt(majorReviewStats.collected)} 个已有可比较专业分，${fmt(majorReviewStats.reviewedWithoutRecords)} 个已核查仍缺可比较专业分，${fmt(majorReviewStats.notReviewed)} 个尚待专业分核查。专业分已核查 ${fmt(majorReviewStats.reviewed)} 个院校代码，不代表专业已完整覆盖。另有 ${fmt(noPlans)} 个尚未收录分专业计划、${fmt(noMajors)} 个仍缺可比较专业录取分。仅有待核分的 ${fmt(majorReviewStats.pendingOnlySchools)} 校计入已核查缺口，待核记录仍可查看。`;
  $('gap-count').textContent=`找到 ${fmt(rows.length)} 个院校条目 · 面向 2026 年；年份等来源冲突单列待核，仍是部分覆盖`;
...
function changeView(next) {
  ...
  document.querySelectorAll('[data-view]').forEach(b=>{const active=b.dataset.view===view || (b.dataset.view==='cutoffs' && ['plans','majorCutoffs'].includes(view));b.classList.toggle('active',active);if(active)b.setAttribute('aria-current','page');else b.removeAttribute('aria-current');});
  $('subview-back').hidden=!['plans','majorCutoffs'].includes(view);
...
$('detail-content').addEventListener('click',e=>{const detail=e.target.closest('[data-detail]'),save=e.target.closest('[data-save]');if(detail)openDetail(detail.dataset.detail);else if(save){saveRow(save.dataset.save);save.textContent=saved.has(save.dataset.save)?'已加备选':'＋ 加入备选';save.setAttribute('aria-pressed',String(saved.has(save.dataset.save)));}else followSchoolLink(e);});
$('reload').addEventListener('click',()=>location.reload());

const names={"cutoffs":"cutoffs","plans":"plans","majorCutoffs":"major-cutoffs","ranks":"ranks","special":"special-programs","sources":"sources","policies":"policies","coverage":"school-coverage","auditNotes":"school-audit-notes","planSources":"plan-source-catalog","supplementaryPlans":"supplementary-plans","majorFiling":"major-filing-cutoffs","groupAdmissions":"group-admission-scores"};
Promise.all(Object.entries(names).map(async([key,file])=>{const response=await fetch(new URL(`./data/${file}.json?v=${DATA_VERSION}`,import.meta.url));if(!response.ok)throw new Error(`${file}: HTTP ${response.status}`);const records=await response.json();if(!Array.isArray(records))throw new Error(`${file}: 数据格式错误`);return [key,records];}))
