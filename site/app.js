...
function render() {
  ...
  $('result-notice').innerHTML = isPlan
    ...
    : isMajor
    ? '<strong>这里展示高校公开的具体专业录取最低分，现为部分覆盖。</strong> 可按录取轮次缩小范围；每条保留首轮、征集或汇总口径，汇总不能当作首轮。未公布组码不猜测。'
...
function openDetail(id) {
  ...
  const showGroupPlans = isCutoff && r.round === '首轮';
  const plans = showGroupPlans ? planIndex.get(exactGroupKey(r)) || [] : [];
  ...
  $('detail-content').innerHTML = `<h2>${h(r.school)}${r.group?' · '+h(r.group)+' 组':''}</h2><div class="detail-grid">${rows.map(([k,v])=>`<div><span>${h(k)}</span><strong>${h(v)}</strong></div>`).join('')}</div>${r.note ? `<div class="notice">${h(r.note)}</div>` : ''}${r.major && r.requiredSubjects ? `<p>再选科目：${r.subjectRule === 'unknown' ? '待核实' : r.requiredSubjects.length ? h(r.requiredSubjects.join(r.subjectRule==='any'?' / ':' + ')) : '不限'}。学费：${h(r.tuition ?? '待核实')}；学制：${h(r.duration ?? '待核实')}。</p>` : ''}${gaps.length ? `<div class="notice"><strong>本专业尚待核实或关联：</strong>${h(gaps.join('、'))}。可在“院校数据缺口”查看该校资料覆盖。</div>` : ''}${showGroupPlans ? `<h3>已采集并匹配到本组的专业计划（${plans.length} 条）</h3><p class="tiny muted">目前是部分计划覆盖；填报前需确认全组专业及调剂范围。</p>${plans.length ? plans.map(p=>`<div class="detail-plan"><strong>${h(p.major)}</strong><p>计划 ${h(p.plannedCount ?? '待核实')} 人 · 学费 ${h(p.tuition ?? '待核实')} · 学制 ${h(p.duration ?? '待核实')}</p><p>${h(p.note || '')}</p>${sourceLinks(p)}</div>`).join('') : '<p>本组还没有可严格匹配的专业计划。请到官方计划系统或高校招生网站核对，不能用相似专业推断。</p>'}` : ''}<h3 style="margin-top:24px">来源与核验</h3>${sourceIds(r).map(id=>{const s=sourceMap.get(id);return s ? `<p>${sourceLink(id,s.title)}<br><span class="tiny muted">发布：${h(s.publishedAt || '原页未标日期')} · 采集：${h(s.accessedAt?.slice(0,10) || '未标')} ${r.sourceRow ? '· 原表数据行 '+h(r.sourceRow) : ''}</span></p>${s.notes?.length ? `<p class="tiny">${h(s.notes.join(' '))}</p>` : ''}` : '';}).join('')}${fieldEvidence ? `<h3>补充字段的核验来源</h3>${fieldEvidence}` : ''}${group ? `<p>组投档线来源：${sourceLinks(group)}</p>` : ''}<p class="help">院校章程中的体检、单科、外语语种、校区、调剂与资格限制仍须逐项核对。2026 年数据不能保证未来录取。</p>`;
  $('detail').showModal();
}
...
$('gap-list').addEventListener('click',e=>{const button=e.target.closest('[data-school-query]');if(!button)return;$('filters').reset();$('query').value=button.dataset.schoolQuery;$('kind').value='all';$('batch').value='all';const school=data.coverage.find(r=>String(r.schoolCode || r.school)===button.dataset.schoolQuery);if(school && !school.tracks.includes($('track').value))$('track').value=school.tracks[0];changeView(button.dataset.destination);$('results').scrollIntoView({block:'start'});});
$('reload').addEventListener('click',()=>location.reload());

const names={cutoffs:'cutoffs',plans:'plans',majorCutoffs:'major-cutoffs',ranks:'ranks',special:'special-programs',sources:'sources',policies:'policies',coverage:'school-coverage',auditNotes:'school-audit-notes'};
