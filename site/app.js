import { getReference, filterRows, exactGroupKey, firstRoundIndex, comparableScore, recordSourceIds, escapeHTML as h, csvCell, safeUrl, subjectStatus, specialLabel } from './logic.js';
...
const scoreAuditLabels = {'collected-with-source-conflict':'已收录，含原文冲突', 'current-year-query-empty':'本年查询返回空列表', 'entry-only-year-gap':'已找到入口，本年专业分仍待核', 'not-found-current-major':'本轮未取得本年专业分', 'empty-current-response':'本年查询返回空列表', 'collected-partial':'已收录部分专业分', collected:'已收录部分专业分', unavailable:'本轮未取得本年专业分', 'source-conflict':'原文冲突待核'};
...
function schoolPlanSources(row) {
  ...
  return `<section class="plan-source-panel"><h3>该校专业计划来源</h3><p>${h(catalog ? planSourceLabels[catalog.status] : '尚未核实该校的公开计划来源')}。${schoolPlans.length ? `本站已收录该校 ${schoolPlans.length} 条计划，含不同科类和招生类别；不代表本组已完整收录。` : '本站尚未收录该校专业计划，这不表示学校没有公布。'}</p>${catalog?.note ? `<p class="tiny">${h(catalog.note)}</p>` : ''}${catalog?.entryUrl ? `<p><a href="${h(safeUrl(catalog.entryUrl))}" target="_blank" rel="noopener noreferrer">打开该校官方招生入口 ↗</a></p>` : ''}${links ? `<details><summary>查看已核实的数据来源</summary><p>${links}</p><p class="tiny muted">核查日期：${h(catalog.checkedAt.slice(0,10))}。查询系统中的部分来源需从该校入口选择年份和广西后查看。</p></details>` : ''}${schoolPlans.length ? `<p><button type="button" data-school-query="${h(row.schoolCode)}" data-destination="plans" data-track="${h(row.track)}">查看该校已收录计划</button></p>` : ''}<p class="tiny"><a href="https://zyfz.gxeea.cn/Main/Jihua/JH_Chaxun_New.aspx" target="_blank" rel="noopener noreferrer">广西官方完整计划查询 ↗</a>（需登录）</p></section>`;
}
...
function openDetail(id) {
  ...
  const fieldLabels={group:'专业组代码',batch:'录取批次',category:'招生类别',tuition:'学费',duration:'学制',requirementText:'选科要求',plannedCount:'计划人数',plannedCountScope:'人数口径'};
