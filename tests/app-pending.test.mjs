...
test('仅有年份冲突待核分的学校留在已核查缺口，记录与详情仍可查看',async()=>{
  ...
  assert.equal(requestedUrls.length,12,'全部实际JSON请求都需要共同版本');
  ...
  assert.equal(document.querySelector('.score').textContent,'原文待核');
  document.querySelector('[data-view="plans"]').click();input('query','10001');document.querySelector('[data-detail]').click();
...
test('真实库全部专业投档记录可由同校同科类详情展开且不混入实际分筛选',async()=>{
  ...
  assert.ok(plan);
  document.querySelector('[data-view="plans"]').click();$('reset').click();input('query',plan.schoolCode);input('track',plan.track);input('kind','all');input('batch','all');
  document.querySelector('[data-detail]').click();assert.ok(document.querySelector('.major-filing-panel'));$('close-detail').click();
  document.querySelector('[data-view="majorCutoffs"]').click();$('reset').click();input('query','14127');input('kind','all');input('batch','all');
